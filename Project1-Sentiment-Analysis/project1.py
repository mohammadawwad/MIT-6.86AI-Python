...
def hinge_loss_full(feature_matrix, labels, theta, theta_0):
    ...
    """
    h_loss_sum = 0  # Initialize the sum of hinge losses
    
    # Iterate over the rows of the feature matrix
    for i in range(feature_matrix.shape[0]):
        h_loss_sum += hinge_loss_single(feature_matrix[i], labels[i], theta, theta_0)
    
    # Calculate the average hinge loss
    average_hinge_loss = h_loss_sum / feature_matrix.shape[0]
...
def average_perceptron(feature_matrix, labels, T):
    ...
    sum_theta = np.zeros(num_features)
    sum_theta_0 = 0.0
    ...
    avg_theta = sum_theta/(num_samples*T)
    avg_theta_0 = sum_theta_0/(num_samples*T)
